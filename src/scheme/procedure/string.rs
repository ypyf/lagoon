...
// 其中k是一个精确非负整数
pub fn make_string(_context: &mut Context, args: Vec<Sexp>) -> LispResult {
    let arity = args.len();
    if arity == 1 {
        match args[0] {
...
pub fn string_length(_context: &mut Context, args: Vec<Sexp>) -> LispResult {
    let arity = args.len();
    if arity != 1 {
...
pub fn string_ref(_context: &mut Context, args: Vec<Sexp>) -> LispResult {
    let arity = args.len();
    if arity != 2 {
...
pub fn string_set(_context: &mut Context, args: Vec<Sexp>) -> LispResult {
    let arity = args.len();
    if arity != 3 {
